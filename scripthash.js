...
function main() {
    ...
    let numeros_decimais;

    listaDosEnderecosLocalizados = [];

    if (arquivo) {
        const file = fs.readFileSync(arquivo, 'utf-8');
        numeros_decimais = file.replace(/\r/g, '').split('\n');
    } else {
        numeros_decimais = [enderecoSolicitado];
    }

    for (const endereco of numeros_decimais) {
        if (endereco != '') {
            let eh32bits = Boolean(enderecoSolicitado > 65535);
            let localizado = null;
            if (eh32bits && usaHierarquia) {
...
function localizarEnderecoHierarquia(enderecoSolicitado, tamanhoDaPagina, eh32bits) {
    ...
    let enderecoBinario = decimalToBinary(enderecoSolicitado).padStart(32, 0);
    ...
    let bitsParaDeslocamento = eh32bits ? 12 : Math.log2(tamanhoDaPagina);
    let bitsParaNiveis = eh32bits ? 10 : 0;
    ...
    for (const numero of numeros_decimais) {
        ...
        if (enderecoBinario == binarioCompleto) {
            ...
            obj.numero_deslocamento = numero_deslocamento;
            obj.valorLido = numero;
        ...
        if (numero_deslocamento >= tamanhoDaPagina) {
            ...
            numero_paginas_nivel2++;
            if (numero_paginas_nivel2 >= limitMaximoHierarquia) {
...
function localizarEndereco(enderecoSolicitado, tamanhoDaPagina, eh32bits) {
    ...
    let numero_deslocamento = 0;
    let tratamentoBinarioDeslocamento = Math.log2(tamanhoDaPagina);
